...
console.log($scope.leadFollowupStatus);
		...
		{
			blockUI.start('Please Wait..');
			dataService.getData('/ws/getLeadManagerHotLeads',{agent:"browser"}).then(function(response){
					recs=response.data;
					if(recs!='0')
					{
		...
		{
			blockUI.start('Please Wait...');
			dataService.getData('/ws/getLeadManagerHotInboxLeads',{agent:"browser"}).then(function(response){
				recs=response.data;
				if(recs!='0')
				{
					angular.forEach(recs,function(val,key){
			...
			if(deptid!=0)
			{
				...
				dataService.getData('/ws/getEditHotLead',{agent:"browser",id:$scope.leadid}).then(function(response){
						...
						if(result)
						{
							...
							$scope.lead.shortlead=$scope.lead.state_id;
							if($scope.lead.shortlead=='0')
							{
								$scope.lead.state_id=23;
								//$scope.lead.state_id='';
							}
							$scope.prospecttype=CONSTANTS.PROSPECTTYPE;
				...
				dataService.getData('/ws/getEditHotLead',{agent:"browser",id:leadid}).then(function(response){
						...
						if(result)
						{
							...
							if($scope.lead.shortlead=='0')
							{
								$scope.lead.state_id='';
								$scope.lead.city_id='';
								$scope.lead.nature_of_business_type='';
								$scope.lead.business_name='';
								$scope.lead.prospect_details_id='';
							}
							$scope.cities=result.recs;
