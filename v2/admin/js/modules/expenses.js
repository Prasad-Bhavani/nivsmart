app.controller('expensesCtrl', function($scope,$http,blockUI,$rootScope,alertService,NgTableParams,$filter,$parse,CONSTANTS,ngDialog,dataService){
	...
	angular.extend($scope,{
		subExpensesStatus:function(formStatus)
		{
			...
			if(formStatus)
			{
				$scope.pay.admin_status=$filter('foreachval')($scope.pay.status);
				$scope.pay.admin_remarks=$filter('foreachval')($scope.pay.remarks);
				dataService.getData('/ws/updateExpensesByAdmin',$scope.pay).then(function(response){
		...
		getViewExpense:function(rec)
		{
			...
			$scope.isView=true;
			$scope.admin_totals=0;
			$scope.admintotal=false;
