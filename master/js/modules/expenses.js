app.controller('expensesCtrl', function($scope,$http,blockUI,$rootScope,alertService,NgTableParams,$filter,$parse,CONSTANTS,ngDialog,dataService){
	...
	angular.extend($scope,{
		subExpensesStatus:function(formStatus)
		{
			...
			if(formStatus)
			{
				$scope.pay.master_status=$filter('foreachval')($scope.pay.status);
				$scope.pay.master_remarks=$filter('foreachval')($scope.pay.remarks);
				dataService.getData('/ws/updateExpensesByMaster',$scope.pay).then(function(response){
		...
		getViewExpense:function(rec)
		{
			...
			$scope.isView=true;
			$scope.master_totals=0;
			$scope.mastertotal=false;
		...
		getTotalAmount:function(rows)
		{
			$scope.mastertotal=true;
			subtotal=0;
			angular.forEach(rows,function(v,k){
				if($scope.pay.status[k]=='Approved') subtotal+=parseInt(v.amount);
			});
			$scope.master_totals=subtotal;
		}
	});
