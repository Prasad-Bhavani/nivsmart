app.controller('leadCtrl', function($scope,$http,blockUI,$rootScope,alertService,NgTableParams,$filter,$parse,CONSTANTS,ngDialog,dataService){
	...
	angular.extend($scope,{
		...
		{
			blockUI.start('Please Wait...');
			dataService.getData('/ws/getFEEDBACKLeads',{agent:"browser"}).then(function(response){
			recs=response.data;
				if(recs!='0')
				{
		...
		{
			blockUI.start('Please Wait...');
			dataService.getData('/ws/getFEEDBACKInboxLeads',{agent:"browser"}).then(function(response){
			recs=response.data;
				if(recs!='0')
				{
					angular.forEach(recs,function(val,key){
		...
		moveToInbox:function(id,status)
		{
			...
			ngDialog.openConfirm({
		   	...
		   	blockUI.start('Please Wait...');
			dataService.getData('/ws/moveToFEEDBACKInbox',{agent:"browser",id:id,status:status}).then(function(response){
		...
		subLeadStatus:function(formStatus)
		{
			...
			{
				blockUI.start('Please Wait...');
				dataService.getData('/ws/updateLeadStatusbyFEEDBACK',$scope.lead).then(function(response){
					result=response.data;
				blockUI.stop();
					if(result)
					{
