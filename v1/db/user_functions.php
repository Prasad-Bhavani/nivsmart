...
if(!empty($_SESSION['nivsmartid']))
{
	...
	{
		return getQueryRecord('select e.emp_name,d.dept,r.role from '.EMP.' e join '.DEPARTMENTS.' d on d.id=e.emp_dept_id join '.DEPTROLES.' r on r.id=e.emp_role_id where e.id='.$_SESSION['nivsmartid']);
	}
}
...
{
	$branch_id=getQueryRecord('select id from '.BRANCHES.' where branch_name="'.$_SESSION['niv_branch'].'"');
		$recs=getQueryRecords('select c.company_name,c.contact_no_1,c.contact_person,l.id,l.lead_id,l.created_date_time,l.status,d.dept as dept,emp.emp_id as is_present from '.CRMLEADS.' l join '.CUSTOMERDETAILS.' c on c.id=l.customer_id LEFT join '.EMP.' emp on emp.id=l.is_present LEFT join '.DEPARTMENTS.' d on d.id=l.lead_dept_id where l.created_emp_id='.$_SESSION['nivsmartid'].' and l.lead_branch_id='.$branch_id['id'].' order by l.id desc');
		if($recs=='') $recs=0;
...
{
	/*$branch_id=getQueryRecord('select id from '.BRANCHES.' where branch_name="'.$_SESSION['niv_branch'].'"');*/
		$recs=getQueryRecords('select c.company_name,c.contact_no_1,c.contact_person,l.id,l.amc_lead_id,l.created_date_time,l.tally_serial_no,l.if_interest_amc from '.AMCLEADS.' l join '.CUSTOMERDETAILS.' c on c.id=l.customer_id where 1 order by l.id desc');
		if($recs=='') $recs=0;
