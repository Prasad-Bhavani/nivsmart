...
if(!empty($_SESSION['nivsmartid']))
{
	...
	if($_GET['path']=='createEMP')
	{
		...
		if($rec==0)
		{
			...
			if(isset($emp_grade_id)) $data['emp_grade_id']=$emp_grade_id;
			...
			$data['emp_adhar_no']=$emp_adhar_no;
			if(isset($emp_pan_no)) $data['emp_pan_no']=$emp_pan_no;
			...
			{
                                $emp_id=substr($emp_id,5,strlen($emp_id));
				$data['emp_id']=$roleid['label'].str_pad($emp_id, 3, 0, STR_PAD_LEFT);
	...
	if($_GET['path']=='AMCLeadCreation')
	{
		...
		$ldata['remarks']=$remarks;
		$ldata['status']=0;
		$ldata['invoice']=$invoice;
		$ldata['inventory']=$inventory;
		$ldata['filing']=$filing;
		$ldata['diagonals']=$diagonals;
	...
	if($_GET['path']=='addTeamTargets')
	{
		foreach ($teamData as $team) {
			$data['team_id']=$team['team_id'];
			$data['target_period']=$team['target_period'];
			$data['months']=$team['targetList']['months'];
			if(empty($id)) $data['reached_targets']=$team['targetList']['reached_targets'];
			$data['targets']=$team['targetList']['targets'];
			$data['total_target']=$team['total_target'];
			$data['created_date_time']=CDATE;
			$id=addRecord(TEAMTARGETS,$data);
		}
		$recs=getQueryRecords('select team.name,targets.* from '.TEAMTARGETS.' targets join '.TEAMS.' team on team.id=targets.team_id order by targets.id desc');
		echo json_encode($recs);
	}
	else
	if($_GET['path']=='getTeamsTargets')
	{
		$recs=getQueryRecords('select team.name,targets.* from '.TEAMTARGETS.' targets join '.TEAMS.' team on team.id=targets.team_id order by targets.id desc');
		if($recs=='') $recs=0;
	...
	if($_GET['path']=='getTeamTargets')
	{
		$rec=getQueryRecord('select team.name,targets.* from '.TEAMTARGETS.' targets join '.TEAMS.' team on team.id=targets.team_id where targets.id='.$id);
		echo json_encode($rec);
	}
	else
	if($_GET['path']=='updateTeamTargets')
	{
		$data['reached_targets']=$reached_targets;
		$data['reached_target']=$reached_target;
		$data['target_updated_date_time']=$target_updated_date_time;
		if($sales_target!='') $data['sales_target']=$sales_target;
		if($bonus!='') $data['bonus']=$bonus;
		if($sales_target!='') $data['sales_target_updated_date']=CDATE;
		if($bonus!='') $data['bonus_updated_date']=CDATE;
		$data['updated_date_time']=CDATE;
		updateRecord(TEAMTARGETS,$data,'id='.$id);

		echo json_encode(1);
	}
	else
	if($_GET['path']=='getTeamsTargetsbyPeriod')
	{
		$period=date('M-Y');
		$rec=getQueryRecord('select target_period from '.TEAMTARGETS.' where months like "%'.$period.'%" group by target_period');
		if($rec=='') $rec=0;

		$recs=getQueryRecords('select team.name,targets.* from '.TEAMTARGETS.' targets join '.TEAMS.' team on team.id=targets.team_id where targets.months like "%'.$period.'%" order by targets.reached_target desc');
		if($recs=='') $recs=0;
		$data=array('rec'=>$rec['target_period'],'recs'=>$recs);
